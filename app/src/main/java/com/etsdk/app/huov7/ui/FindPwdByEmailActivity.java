...
import com.etsdk.app.huov7.model.SmsMailRequestBean;
...
public class FindPwdByEmailActivity extends ImmerseActivity {
    ...
    private void setupUI() {
        ...
        tvTitleName.setText("密保邮箱找回");
        btnSubmit.setText("提交");
        username = getIntent().getStringExtra("username");
    }

    public static void start(Context context, String username) {
        Intent starter = new Intent(context, FindPwdByEmailActivity.class);
    ...
    private void submitUpdatePassword() {
        ...
        pwdFindRequestBean.setEmail(huoSdkEtAccount.getText().toString().trim());
    ...
    private void sendSms() {
        ...
        if(TextUtils.isEmpty(account)){
            T.s(mContext,"请输入正确的邮箱号");
            return;
        }
        final SmsMailRequestBean accountInfoRequestBean = new SmsMailRequestBean();
        accountInfoRequestBean.setEmail(account);
        accountInfoRequestBean.setType(SmsSendRequestBean.TYPE_FIND_PWD);
        ...
        RxVolley.post(AppApi.getUrl(AppApi.sendUserEmailApi), httpParamsBuild.getHttpParams(),httpCallbackDecode);
    }
