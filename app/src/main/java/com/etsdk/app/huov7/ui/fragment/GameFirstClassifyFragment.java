...
public class GameFirstClassifyFragment extends AutoLazyFragment {
    ...
    private void setupUI() {
        ...
        firstClassifyAdapter = new GameFirstClassifyAdapter(datas);
        // 设置适配器
    ...
    private void getNetData() {
        ...
        NetRequest.request(this).setParams(httpParams).get(AppApi.getUrl(AppApi.gametypeApi), new HttpJsonCallBackDialog<GameClassifyListModel>() {
            @Override
            public void onDataSuccess(GameClassifyListModel data) {
                if (data != null && data.getData() != null && data.getData().getList() != null) {
                    firstClassifyAdapter.setDatas(data.getData().getList());
                    swipeRefreshLayout.setRefreshing(false);
                    loadview.showSuccess();
                }else{
                    firstClassifyAdapter.setDatas(new ArrayList<GameClassifyListModel.GameClassify>());
                    swipeRefreshLayout.setRefreshing(false);
                    loadview.showSuccess();
                }
            }
