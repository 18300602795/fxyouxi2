...
public class GameFirstClassifyAdapterV1 extends RecyclerView.Adapter {
    ...
    public GameFirstClassifyAdapterV1(List<GameClassifyListModelV1.GameClassify> datas) {
        this.datas = datas;
    }
...
//            itemViewHolder.secondClassifyRecycler.addItemDecoration(new RecommandOptionRcyDivider(context, LinearLayoutManager.VERTICAL,2, Color.parseColor("#e5e5e5")));
            final GameClassifyListModelV1.GameClassify gameClassify = datas.get(index);
...
//            GlideDisplay.display(itemViewHolder.ivFirstClassifyImg, gameClassify.getIcon(), R.mipmap.icon_load);
            Glide.with(itemViewHolder.context).load(gameClassify.getIcon()).placeholder(R.mipmap.icon_load).into(itemViewHolder.ivFirstClassifyImg);
...
//                    for(GameClassifyListModel.GameClassify data: gameClassify.getSublist()){
//                        if(data != null){
//                            tempList.add(data);
//                        }
//                    }
//                    ClassifyGameListActivity.start(v.getContext(), gameClassify.getTypename(), GsonUtil.getGson().toJson(tempList), 0);
    ...
    public List<GameClassifyListModelV1.GameClassify> getDatas() {
        return datas;
    }

    public void setDatas(List<GameClassifyListModelV1.GameClassify> datas) {
        this.datas.clear();
